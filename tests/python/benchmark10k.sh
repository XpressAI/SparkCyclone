...
run_benchmark.py  --outputfile "yarn_test_ve_10K" --clearcache --ntest 5 \
random "data/XY_doubles_R10000_P100_csv" -t column \
--list "sum_float,avg_float,(x+y)_float,avg(x+y)_float,sum(x+y)_float"

# GPU 
/opt/spark/bin/spark-submit --master yarn \
--name GPU_Benchmark_random_10K \
--deploy-mode cluster \
--py-files dep.zip \
...
run_benchmark.py  --outputfile "yarn_test_gpu_10K" --clearcache --ntest 5 \
random "data/XY_doubles_R10000_P100_csv" -t column \
--list "sum_float,avg_float,(x+y)_float,avg(x+y)_float,sum(x+y)_float"
...
/opt/spark/bin/spark-submit --master yarn \
--deploy-mode cluster \
--name CPU_Benchmark_random_10K \
--py-files dep.zip \
--num-executors=24 --executor-cores=2 --executor-memory=16G \
run_benchmark.py  --outputfile "yarn_test_cpu_10K" --clearcache --ntest 5 \
random "data/XY_doubles_R10000_P100_csv" -t column \
--list "sum_float,avg_float,(x+y)_float,avg(x+y)_float,sum(x+y)_float"
