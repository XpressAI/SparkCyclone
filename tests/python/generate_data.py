...
def main(args):
    ...
    df = (spark.range(0, args.rows, numPartitions=args.partitions)
        ...
        .withColumn('float_x', F.rand(seed=8675309)*10000)
        .withColumn('float_y', F.rand(seed=8675367)*1000)
        .withColumn('int_x', F.col('float_x').cast(T.IntegerType()))
        .withColumn('int_y', F.col('float_y').cast(T.IntegerType()))
        .withColumn('float_a', F.rand(seed=8675309)*100)
        .withColumn('float_b', F.rand(seed=8675367)*10)
        .withColumn('int_a', F.col('float_a').cast(T.IntegerType()))
        .withColumn('int_b', F.col('float_b').cast(T.IntegerType()))
