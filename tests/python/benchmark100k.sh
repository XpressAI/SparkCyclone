...
random "data/XY_doubles_R100000_P100_csv" -t column \
--list "sum_float,avg_float,(x+y)_float,avg(x+y)_float,sum(x+y)_float"

# GPU 
/opt/spark/bin/spark-submit --master yarn \
--name GPU_Benchmark_random_100K \
--deploy-mode cluster \
--py-files dep.zip \
...
run_benchmark.py  --outputfile "yarn_test_gpu_100K" --clearcache --ntest 5 \
random "data/XY_doubles_R100000_P100_csv" -t column \
--list "sum_float,avg_float,(x+y)_float,avg(x+y)_float,sum(x+y)_float"
...
/opt/spark/bin/spark-submit --master yarn \
--deploy-mode cluster \
--name CPU_Benchmark_random_100K \
--py-files dep.zip \
