    ...
    _.config(CODEGEN_FALLBACK.key, value = false)
      ...
      .withExtensions(sse =>
        sse.injectPlannerStrategy(sparkSession => {
          ...
          new VERewriteStrategy(CNativeEvaluator(debug = false), VeRewriteStrategyOptions.default.copy(preShufflePartitions = None))
        })
      )
  }
...
class DynamicCSqlExpressionEvaluationSpec
  ...
  "Support order by with select with addition" ignore withSparkSession2(configuration) { sparkSession =>
    makeCsvNumsMultiColumn(sparkSession)
    import sparkSession.implicits._
