...
class LruVeoMemCache(val maxSize: Int) {
    private val map: LinkedHashMap[(Aurora.veo_proc_handle, String), Long] = new LinkedHashMap[(Aurora.veo_proc_handle, String), Long](maxSize, 0.75f, true) {
        override def removeEldestEntry(e: java.util.Map.Entry[(Aurora.veo_proc_handle, String), Long]): Boolean = {
            if (size > maxSize) {
