    ...
    CodeLines.from(
      ...
      CodeLines
        .from(
          ...
          s"""std::vector<size_t> ${a1_idx}(${a1}.size());""",
          CodeLines.forLoop("i", "a1.size()")(CodeLines.from(s"""${a1_idx}[i] = i;""")),
          s"""std::vector<size_t> ${a2} = ${x_a_dict}.lookup(${y_a_cwords});""",
          s"""std::vector<size_t> ${a2_idx}(${a2}.size());""",
          CodeLines.forLoop("i", s"${a2}.size()")(CodeLines.from(s"""${a2_idx}[i] = i;""")),
          s"""std::vector<size_t> ${a1_out};""",
          s"""std::vector<size_t> ${a2_out};""",
          s"""frovedis::equi_join(${a2}, ${a2_idx}, ${a1}, ${a1_idx}, ${a2_out}, ${a1_out});""",
          s"""std::vector<int64_t> ${b1}(${x_b}->count);""",
          s"""std::vector<size_t> ${b1_idx}(${x_b}->count);""",
          CodeLines.forLoop(counterName = "i", until = s"${x_b}->count")(
            CodeLines.from(s"""${b1}[i] = ${x_b}->data[i];""", s"""${b1_idx}[i] = i;""")
          ),
          s"""std::vector<int64_t> ${b2}(${y_b}->count);""",
          s"""std::vector<size_t> ${b2_idx}(${y_b}->count);""",
          CodeLines.forLoop(counterName = "i", until = s"${y_b}->count")(
            CodeLines.from(s"""b2[i] = ${y_b}->data[i];""", s"""${b2_idx}[i] = i;""")
          ),
          ...
          CodeLines.forLoop(counterName = "i", until = s"${a1_out}.size()")(
            CodeLines.forLoop(counterName = "j", until = s"${b1_out}.size()")(
              CodeLines.ifStatement(condition = s"""${a1_out}[i] == ${b1_out}[j]""")(
                CodeLines.ifStatement(condition = s"""${a2_out}[i] == ${b2_out}[j]""")(
                  CodeLines
                    .from(
