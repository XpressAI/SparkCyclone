...
object StringProducer {
  ...
  final case class FilteringProducer(outputName: String, stringProducer: StringProducer) {
    ...
    def complete: CodeLines = CodeLines.from(
      CodeLines.debugHere,
      s"""${tmpData}_new_starts.resize(${tmpData}_new_starts.size() - 1);""",
      s"""${tmpData}_new_lens.resize(${tmpData}_new_lens.size() - 1);""",
      s"""std::vector<size_t> ${tmpData}_new_new_starts;""",
      """std::cout << "Calling concat_words" << std::endl; """,
      s"""debug_words(${tmpData});""",
      s"""std::vector<int> ${tmpData}_new_chars;""",
      s"""try {""",
      s"""    ${tmpData}_new_chars = frovedis::concat_words(${tmpData}.chars, ${tmpData}_new_starts, ${tmpData}_new_lens, "", ${tmpData}_new_new_starts);""",
      s"""}""",
      s"""catch (...) { std::cout << "Caught exception!!!!" << std::endl;""",
      s"""    for (int i = 0; i < ${tmpData}_new_starts.size(); i++) {""",
      s"""        std::cout << "${tmpData}_new_starts[" << i << "] = " << ${tmpData}_new_starts[i] << std::endl;""",
      s"""        std::cout << "${tmpData}_new_lens[" << i << "] = " << ${tmpData}_new_lens[i] << std::endl;""",
      s"""    }""",
      s"""}""",
