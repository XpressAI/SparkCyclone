...
final class ArrowSummingCodegenPlanSpec
  ...
  with SparkAdditions
  with Matchers {
  "Execute WSCG for a SUM() VeSummingCodegenPlan" in withSparkSession2(
  ...
  "Execute WSCG for a SUM() VeSummingCodegenPlan with Columnar/Parquet input" in withSparkSession2(
    _.config(CODEGEN_FALLBACK.key, value = false)
      .config("spark.sql.codegen.comments", value = true)
      .withExtensions(sse =>
        sse.injectColumnar(ss =>
  ...
  "Execute WSCG for a SUM() VeSummingCodegenPlan with a CSV input" in withSparkSession2(
    _.config(CODEGEN_FALLBACK.key, value = false)
      .config("spark.sql.codegen.comments", value = true)
      .withExtensions(sse =>
        sse.injectColumnar(ss =>
