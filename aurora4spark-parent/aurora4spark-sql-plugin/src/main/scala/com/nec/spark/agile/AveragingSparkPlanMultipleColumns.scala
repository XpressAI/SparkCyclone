...
object AveragingSparkPlanMultipleColumns {
  ...
  val averageLocalScala: ColumnWithNumbers => Double = l => l match {
  ...
  def averagingRdd(parentRdd: RDD[ColumnWithNumbers],
                   f: ColumnWithNumbers => Double): RDD[Seq[Double]] =
    parentRdd
      .coalesce(1)
      .mapPartitions(its => {
...
final case class AveragingSparkPlanMultipleColumns(child: SparkPlan,
