# C++ Data Lifetime Management

## Status
PROPOSED

Proposed by: Carlos Reyes (February 25, 2022)

Discussed with:

* Benson Ma (early ideas)
* Eduardo Gonzalez (overall design goals, feedback on draft ADR)

## Context
Our architecture has a low-level C/C++ layer and a high level Java/Scala layer. We use JavaCpp to communicate between
the layers. We would like to only use C++ code in the low level layer. Right now we have a mix of C and C++ code.

We allocate the data buffers passed to the low-level layer using Scala. These buffers are freed using Scala code
after processing is done. We allocate return values using C++ and deallocate them from Scala.
This requires us to use C-style calloc/free system calls.

## Motivation

We would like to migrate away from this design.
...
### Sample Code - Output
```
before malloc                                                                                                                                                                                                                               before placement new                                                                                                                                                                                                                        
Data()                                                                                                                                                                                                                                      
before print()                                                                                                                                                                                                                              
123     hello                                                                                                                                                                                                                               
before destructor                                                                                                                                                                                                                           
